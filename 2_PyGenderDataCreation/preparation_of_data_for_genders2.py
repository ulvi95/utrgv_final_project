...
if __name__ == "__main__":
    ...
    textfile_log = folder_path + "logFile4_2.txt"

    if os.path.exists(textfile_log):
        file_results_log = open("logFile4_2.txt","a+")
        file_results_log.write(str(datetime.datetime.now()) + "\n")
        file_results_log.close()
    else:
        file_results_log = open("logFile4_2.txt","w+")
        file_results_log.write(str(datetime.datetime.now()) + "\n")
        file_results_log.close()
    ...
    for row in surnames_names_only.iterrows():
        if (row[1]["Surname"][-2:] == "OV") or (row[1]["Surname"][-2:] == "EV"):
            row[1]["Gender"] = "M"
        elif (row[1]["Surname"][-3:] == "OVA") or (row[1]["Surname"][-3:] == "EVA") or (row[1]["Name"][-1:] == "Ə"):
            row[1]["Gender"] = "F"
        else:
            row[1]["Gender"] = "CBD"
        if ((row[0]+1) % 1000 == 0):
