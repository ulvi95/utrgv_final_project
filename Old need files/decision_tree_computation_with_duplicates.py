...
if __name__ == "__main__":
    ...
    csv_decision_tree_results_gini = folder_path_one_level_up + os.sep + "Results" + os.sep + "Decision_Tree_results_gini.csv"
    csv_decision_tree_results_entropy = folder_path_one_level_up + os.sep + "Results" + os.sep + "Decision_Tree_results_entropy.csv"
        ...
        for splitting_criteria in splitting_criterias:
            for variable in variables_to_be_tested:
                ...
                for test_percent in range(99, 0, -1):
                    ...
                    text = "Accuracy of Decision tree with splitting criterion '" + splitting_criteria + "' for " + str(get_variable_name(variable, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(file.split(os.sep)[-1]) + " is: " + str(decision_tree_accuracy_score)
                    print(text)
                    file_results_log = open("logFile5.txt","a+")
                    file_results_log.write(text + "\n")
                    file_results_log.close()
    ...
    for i in range(0, 24, 3):
        ...
        if i<12:
            text = Titles[i//3] + " (Splitting criteria=" + splitting_criterias[i//12] + ")"
            ax.set_title(text, fontsize=16)
            image_with_plotters = folder_path_one_level_up + os.sep + "Results" + os.sep + Titles[i//3].replace(" ", "_").replace("\"", "_") + "_" + splitting_criterias[i//12] + ".png"
            ...
            text = Titles[(i//3)-4] + " (Splitting criteria=" + splitting_criterias[i//12] + ")"
            ax.set_title(text, fontsize=16)
            image_with_plotters = folder_path_one_level_up + os.sep + "Results" + os.sep + Titles[(i//3)-4].replace(" ", "_").replace("\"", "_") + "_" + splitting_criterias[i//12] + ".png"
            matplotlib.pyplot.savefig(image_with_plotters, bbox_inches=ax.get_window_extent().transformed(fig.dpi_scale_trans.inverted()).expanded(1.3, 1.2))