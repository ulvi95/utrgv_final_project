...
if __name__ == "__main__":
    ...
    csv_decision_tree_results_entropy = folder_path_one_level_up + os.sep + "Results" + os.sep + "Decision_Tree_results_entropy_surname_name_M_F_only_2.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        text = "Accuracy of Decision tree with splitting criterion 'entropy' for " + str(get_variable_name(surname_and_name, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(decision_tree_accuracy_score)
        print(text)
        file_results_log = open("logFile5.txt","a+")
        file_results_log.write(text + "\n")
        file_results_log.close()
        accuracy_score.append(decision_tree_accuracy_score)
    accuracy_score_max = "Maximum accuracy of Decision tree with and splitting criterion 'entropy' for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(max(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(max(accuracy_score))+1) + "%"
    ...
    accuracy_score_min = "Minimum accuracy of Decision tree with splitting criterion 'entropy' for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(min(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(min(accuracy_score))+1) + "%"
    ...
    file_results_log2.close()
    accuracy_score_average = "Average accuracy of Decision tree with splitting criterion 'entropy' for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(round((sum(accuracy_score) / len(accuracy_score)),2))
