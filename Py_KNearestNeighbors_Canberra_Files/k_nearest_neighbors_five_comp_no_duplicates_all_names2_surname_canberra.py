...
if __name__ == "__main__":
    ...
    csv_K_Nearest_Neighbours_Five_results_no_duplicates = folder_path_one_level_up + os.sep + "Results" + os.sep + "K_Nearest_Neighbours_Five_results_no_duplicates_surnames_only_2_Canberra.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        text = "Accuracy of K Nearest Neighbours with " + str(n_neigbors) + " neighbors and Canberra distance " + "for " + str(get_variable_name(surnames_only, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(csv_with_all_surnames.split(os.sep)[-1]) + " is: " + str(K_Nearest_Neighbours_Five_accuracy_score)
        print(text)
        file_results_log = open("logfile9_2.txt","a+")
        file_results_log.write(text + "\n")
        file_results_log.close()
