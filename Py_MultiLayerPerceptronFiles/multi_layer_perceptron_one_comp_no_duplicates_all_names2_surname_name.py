...
if __name__ == "__main__":
    ...
    csv_Multi_Layer_Perceptron_OneLayer_results_no_duplicates = folder_path_one_level_up + os.sep + "Results" + os.sep + "Multi_Layer_Perceptron_OneLayer_results_no_duplicates_surname_name_2.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        classifier_Multi_Layer_Perceptron.add(Dense(len(x_train)//2, input_dim=2, activation='sigmoid'))
    ...
    accuracy_score_max = "Maximum accuracy of Multi-Layer Perceptron with one layers for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_all_surnames.split(os.sep)[-1]) + " is: " + str(max(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(max(accuracy_score))+1) + "%"
    ...
    accuracy_score_min = "Minimum accuracy of Multi-Layer Perceptron with one layers for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_all_surnames.split(os.sep)[-1]) + " is: " + str(min(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(min(accuracy_score))+1) + "%"
    ...
    file_results_log2.close()
    accuracy_score_average = "Average accuracy of Multi-Layer Perceptron with one layers for " + str(get_variable_name(surname_and_name, locals())[0]) + " in " + str(csv_with_all_surnames.split(os.sep)[-1]) + " is: " + str(round((sum(accuracy_score) / len(accuracy_score)),2))
