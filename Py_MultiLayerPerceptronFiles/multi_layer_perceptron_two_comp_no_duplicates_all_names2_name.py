...
if __name__ == "__main__":
    ...
    csv_Multi_Layer_Perceptron_TwoLayer_results_no_duplicates = folder_path_one_level_up + os.sep + "Results" + os.sep + "Multi_Layer_Perceptron_TwoLayer_results_no_duplicates_names_only_2.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        classifier_Multi_Layer_Perceptron.add(Dense(len(x_train)//4, input_dim=1, activation='sigmoid'))
        classifier_Multi_Layer_Perceptron.add(Dense(len(x_train)//4, activation='sigmoid'))
        ...
        text = "Accuracy of Multi-Layer Perceptron with one layers for " + str(get_variable_name(names_only, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(csv_with_all_surnames.split(os.sep)[-1]) + " is: " + str(Multi_Layer_Perceptron_TwoLayer_accuracy_score)
        print(text)
        file_results_log = open("logfile11.txt","a+")
        file_results_log.write(text + "\n")
        file_results_log.close()
    ...
    (pd.DataFrame(total_accuracy_scores)).to_csv(csv_Multi_Layer_Perceptron_TwoLayer_results_no_duplicates, header=False, index=False)
