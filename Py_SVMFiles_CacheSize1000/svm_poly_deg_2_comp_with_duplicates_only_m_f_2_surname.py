...
if __name__ == "__main__":
    ...
    csv_SVM_Poly_Degree_2_results = folder_path_one_level_up + os.sep + "Results" + os.sep + "SVM_Poly_Degree_2_results_surnames_only_M_F_only_2.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        classifier_SVM_Poly_Degree_2 = SVC(cache_size=1000, kernel='poly', degree=2)
        classifier_SVM_Poly_Degree_2 = classifier_SVM_Poly_Degree_2.fit(x_train, y_train)
        y_pred_SVM_Poly_Degree_2 = classifier_SVM_Poly_Degree_2.predict(x_test)
        SVM_Poly_Degree_2_accuracy_score = round(metrics.accuracy_score(y_test, y_pred_SVM_Poly_Degree_2), 3)
        text = "Accuracy of SVM Poly_Degree_Two for " + str(get_variable_name(surnames_only, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(SVM_Poly_Degree_2_accuracy_score)
        print(text)
        file_results_log = open("logFile7.txt","a+")
        file_results_log.write(text + "\n")
        file_results_log.close()
        accuracy_score.append(SVM_Poly_Degree_2_accuracy_score)
    ...
    (pd.DataFrame(total_accuracy_scores)).to_csv(csv_SVM_Poly_Degree_2_results, header=False, index=False)
