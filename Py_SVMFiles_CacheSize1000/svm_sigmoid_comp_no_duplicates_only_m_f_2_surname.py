...
if __name__ == "__main__":
    ...
    csv_SVM_Sigmoid_results_no_duplicates = folder_path_one_level_up + os.sep + "Results" + os.sep + "SVM_Sigmoid_results_no_duplicates_surnames_only_M_F_only_2.csv"
    ...
    for test_percent in range(99, 0, -1):
        ...
        text = "Accuracy of SVM Sigmoid for " + str(get_variable_name(surnames_only, locals())[0]) + " with training data fraction " + str(100-test_percent) + "% in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(SVM_Sigmoid_accuracy_score)
        print(text)
        file_results_log = open("logFile7.txt","a+")
        file_results_log.write(text + "\n")
        file_results_log.close()
        accuracy_score.append(SVM_Sigmoid_accuracy_score)
    accuracy_score_max = "Maximum accuracy of SVM Sigmoid for " + str(get_variable_name(surnames_only, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(max(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(max(accuracy_score))+1) + "%"
    ...
    accuracy_score_min = "Minimum accuracy of SVM Sigmoid for " + str(get_variable_name(surnames_only, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(min(accuracy_score)) + " with training data fraction " + str(accuracy_score.index(min(accuracy_score))+1) + "%"
    ...
    file_results_log2.close()
    accuracy_score_average = "Average accuracy of SVM Sigmoid for " + str(get_variable_name(surnames_only, locals())[0]) + " in " + str(csv_with_M_F_surnames.split(os.sep)[-1]) + " is: " + str(round((sum(accuracy_score) / len(accuracy_score)),2))
